...
This file briefs Codex (and future-me) on the project's architecture,
...
Stage 1 — Search & Fetch          tools: search_web, fetch_url
  ...
  │  Content is captured by the loop, NOT echoed through Codex.
  │
  ▼
...
**External state, not round-tripped state.** When Codex calls `fetch_url`, the
agent loop intercepts the result and stashes content in a `Map<url, content>`.
Codex only needs to confirm which URLs to keep — it doesn't have to echo tens
...
**Errors return to Codex as `is_error: true` tool results, not exceptions.**
